...
- **`hub-api` y `nginx` no tenían `healthcheck` en `docker-compose.yml`/`feed-builder.yml`** (solo `hub-service` lo tenía), pese a que ambos ya exponen `/healthz/liveness` (directo y vía proxy). Agregado un `healthcheck` a cada uno (`python -c "import urllib.request..."` para `hub-api`, ya que `python:3.12-slim` no trae `curl`/`wget`; `curl -fsk https://localhost/healthz/liveness` para `nginx`, que sí lo trae y de paso ejercita TLS + el reverse-proxy en un solo golpe) y `nginx` ahora espera `hub-api: condition: service_healthy` en vez de solo `service_started`.

Verificación tras estos cambios: `pytest` (473 tests), `ng build`, `ng test`, `npx playwright test` y `docker compose config` (ambos archivos) en verde.
